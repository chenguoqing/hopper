...
public class BallotGenerator {
     ...
     *
     * @param serverBallotId server ballot id that will generate ballot(0<=serverBallotId<=
     *                       <tt>serverCount</tt>-1)
     * @param serverCount    total server count
     * @param initialBallot  the initial ballot, the new ballot should great than it.
     * @return new ballot
     */
    public static final int generateBallot(int serverBallotId, int serverCount, int initialBallot) {

        if (initialBallot <= 0) {
            return serverBallotId + serverCount;
        }
