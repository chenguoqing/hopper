...
import com.hopper.util.Constants;
...
/**
 * The sub class of LifecycleMBeanProxy represents a MBean, it will register
 * current component when initializing
 */
public abstract class LifecycleMBeanProxy extends LifecycleProxy implements
		MBeanRegistration {
	/**
	...
	protected void doInit() throws Exception {
		if (objectName == null) {
			this.mBeanServer = Registry.getRegistry(null, null)
					.getMBeanServer();
	 ...
	 * 
	 * @param component
	 *            the component instance will be registered
	 * @param objectName
	 *            ObjectName
	 */
	protected final void register(Object component, ObjectName objectName)
			throws Exception {
		if (component == null || objectName == null) {
			throw new NullPointerException("component and objectName be null.");
		}

		Registry.getRegistry(null, null).registerComponent(component,
				objectName, null);
	}
	...
	/**
	 * Create a {@link ObjectName} instance by domain and user-supported name
	 * properties
	 */
	protected ObjectName createObjectName(String nameProperties)
